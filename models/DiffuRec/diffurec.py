...
def get_named_beta_schedule(schedule_name, num_diffusion_timesteps):
    ...
    if schedule_name == "linear":
        ...
        beta_end = scale * 0.02
        return np.linspace(beta_start, beta_end, num_diffusion_timesteps, dtype=np.float32)
        ...
        beta_end = scale * 0.02
        first_part = np.linspace(beta_start, beta_mid, 10, dtype=np.float32)
        second_part = np.linspace(beta_mid, beta_end, num_diffusion_timesteps - 10 , dtype=np.float32)
