...
from modules import TransformerEncoder, in_batch_negative_sampling, extract_axis_1, SinusoidalTimeEmbedding, RFFTimeEmbedding, RBFTimeEmbedding
...
class SASRec(AbstractModel):
    def __init__(self, config: dict):
        ...
        if encoding_type == 'positional':
            print("Using absolute positional embeddings.")
            ...
            print("Using sinusoidal time embeddings.")
            self.temporal_embeddings = SinusoidalTimeEmbedding(config['hidden_size'])
        elif encoding_type == 'RFF':
            print("Using RFF embeddings.")
            self.temporal_embeddings = RFFTimeEmbedding(config['hidden_size'], config['sigma'])
        elif encoding_type == 'Gaussian':
            print("Using Gaussian embeddings.")
            self.temporal_embeddings = RBFTimeEmbedding(config['hidden_size'], config['sigma'])
        elif encoding_type == 'NA' or encoding_type == 'none':
            print("No position or time embeddings will be used.")
    ...
    def get_representation(self, batch):
        ...
        if self.config['encoding_type'] == 'positional':
            inputs_emb += self.positional_embeddings(
            torch.arange(self.config['max_seq_length']).to(inputs_emb.device)
            )
        elif self.config['encoding_type'] == 'sinusoidal':
            timestamps = batch['timestamps_seqs'].to(inputs_emb.device)
            # 处理零值填充
            time_mask = (batch['timestamps_seqs'] > 0).float().unsqueeze(-1).to(inputs_emb.device)
            time_embeddings = self.temporal_embeddings(timestamps)
            inputs_emb += time_embeddings * time_mask
        elif self.config['encoding_type'] == 'RFF' or self.config['encoding_type'] == 'Gaussian':
            timestamps = batch['norm_timestamps_seqs'].to(inputs_emb.device)
            # 处理零值填充
            time_mask = (batch['norm_timestamps_seqs'] > 0).float().unsqueeze(-1).to(inputs_emb.device)
            time_embeddings = self.temporal_embeddings(timestamps)
            inputs_emb += time_embeddings * time_mask
        elif self.config['encoding_type'] == 'NA' or self.config['encoding_type'] == 'none':
        # 不添加任何编码，直接使用原始的item embeddings
            pass
