...
class RFFTimeEmbedding(nn.Module):
    """
    基于随机傅里叶特征 (RFF) 的时间戳编码：
      γ(t) = [cos(2π t b_j), sin(2π t b_j)]_{j=1..M/2},
    其中 b_j ~ N(0, σ^2) 是随机频率。
    """
    def __init__(self, embedding_dim: int, sigma=0.05):
        """
        Args:
            embedding_dim: 输出 embedding 维度 M（必须是偶数）
            sigma: RFF 中频率分布的标准差
        """
        super().__init__()
        ...
        self.sigma = sigma
        # 随机采样频率 b_j ~ N(0, σ^2)，形状 (half_dim, 1)
        b = torch.randn(half_dim, 1) * sigma
        self.register_buffer("b", b)  # 固定不训练

    def forward(self, timestamps: torch.Tensor):
        """
        Args:
            timestamps: Tensor of shape (B, L) 或 (B, L, 1)，
                        值最好归一到 [0,1] 或其他合理区间
        Returns:
            Tensor of shape (B, L, M) 的 RFF embedding
        """
        # 如果输入是 (B, L)，则扩成 (B, L, 1)
        if timestamps.dim() == 2:
        ...
        # 计算 2π t b^T  → (B, L, half_dim)
        proj = 2 * np.pi * t @ self.b.T

        # 拼接 cos & sin → (B, L, M)
        emb = torch.cat([torch.cos(proj), torch.sin(proj)], dim=-1)
        return emb

class RBFTimeEmbedding(nn.Module):
    """基于高斯径向基函数(RBF)的时间戳编码"""
    
    def __init__(self, embedding_dim, sigma=0.001): #1
        """
        Args:
            embedding_dim: 嵌入向量维度，等于中心点数量
            sigma: 高斯核宽度参数，控制"覆盖半径"
        """
        super().__init__()
        ...
        # 在[0,1]范围内均匀分布中心点
        centers = torch.linspace(0, 1, embedding_dim).view(1, 1, -1)
        self.register_buffer('centers', centers)
        
    def forward(self, timestamps):
        """
        Args:
            timestamps: 形状为 [batch_size, seq_len] 的时间戳张量
                       假设已归一化到[0,1]区间
        Returns:
            形状为 [batch_size, seq_len, embedding_dim] 的RBF时间编码
        """
        # 确保时间戳至少是2D的
        if timestamps.dim() == 1:
        ...
        squared_diff = (timestamps - self.centers) ** 2
        
        # 应用RBF公式: exp(-d²/(2σ²))
        # [batch_size, seq_len, embedding_dim]
        embeddings = torch.exp(-0.5 * squared_diff / (self.sigma ** 2)) 
...
class SinusoidalTimeEmbedding(nn.Module):
    ...
    def forward(self, timestamps):
        ...
        # 创建频率索引 i = 0, 1, 2, ..., d/2-1
        # 每个频率对应两个维度（sin和cos）
        half_dim = self.embedding_dim // 2
        frequency_idx = torch.arange(0, half_dim, dtype=torch.float32, device=device)
        
        # 计算频率的除数
        # 根据Transformer公式: PE_{2i} = sin(timestamp / max_period^(2i/d))
        # div_term = 1 / max_period^(2i/d) = exp(-2i * log(max_period) / d)
        div_term = torch.exp(-2 * frequency_idx * math.log(self.max_period) / self.embedding_dim)
        
        # 扩展时间戳维度以便广播
        # timestamps: [batch_size, seq_len] -> [batch_size, seq_len, 1]
        expanded_timestamps = timestamps.unsqueeze(-1)
        
        # 计算相位
        # phases: [batch_size, seq_len, d/2]
        phases = expanded_timestamps * div_term
        
        # 创建输出张量
        embeddings = torch.zeros(batch_size, seq_len, self.embedding_dim, device=device)
        
        # 按照Transformer的方式填充：偶数位置用sin，奇数位置用cos
        embeddings[..., 0::2] = torch.sin(phases)  # 偶数索引: 0, 2, 4, ...
        embeddings[..., 1::2] = torch.cos(phases)  # 奇数索引: 1, 3, 5, ...
        
        return embeddings

"""
#DreamRec和PreferDiff原始用的正余弦函数编码
...
class SinusoidalTimeEmbedding(nn.Module):
    ...
    def forward(self, timestamps):
        ...
        # 计算不同频率的除数
        # 从1到max_period的对数空间划分
        emb_range = torch.arange(half_dim, device=device).float()
        div_term = torch.exp(emb_range * (-math.log(self.max_period) / half_dim))
        
        # 扩展时间戳维度以便广播
        # 最终形状: [batch_size, seq_len, half_dim]
        timestamps = timestamps.unsqueeze(-1)
        
        # 计算正弦和余弦值
        # phases: [batch_size, seq_len, half_dim]
        phases = timestamps * div_term
        ...
        embeddings[..., :half_dim] = torch.sin(phases)
        embeddings[..., half_dim:] = torch.cos(phases)
